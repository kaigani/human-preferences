...
- **Right & wrong** (kind `moral` → `morality` region): moral-tradeoff dilemmas built on Moral Foundations Theory — each pair pits two foundations (care/fairness/loyalty/authority/purity/liberty) against each other and is tagged with both (`entity_tags`), so a Haidt-style foundations profile is computable later. `moral_tradeoff_v1` runner prompt; option_a honors the first foundation in the axis, option_b the second. Pairs can carry `tags` through ingest now.
- **Self & becoming** (kind `identity` → `self` region): self-concept choices. Themes `self-archetype` ("which is more you?"), `self-rather` ("would you rather be?") = current-self; `self-future` ("who do you want to become?") = future-self (the time-horizon framing). `identity_choice_v1` runner prompt.
- **Inner life** (kind `inner` → `inner` region): affective/interpersonal fingerprint. Themes `inner-emotion` ("which moves you more?"), `inner-aversion` (what you'd rather avoid — the choice marks the stronger aversion), `inner-boundary` (closeness/conflict). `inner_life_v1` runner prompt.
- **What shaped you** (kind `memory` → `memory` region, theme `memory-salience`): A/B-ranks the user's OWN life events to find which memories are load-bearing — the bridge from the biography layer to the preference engine (`server/memory.ts`, `refreshMemoryPairs()` auto-fires on life-event create + `POST /api/memory/refresh`). **Contains private biography content → EXCLUDED from the preference exports** (`export/build.ts` filters `t.kind != 'memory'`). Needs ≥2 life events.
